...
$this->menu=array(
	...
	array('label'=>Yii::t('app', 'Update') . ' Teacher', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>Yii::t('app', 'Delete') . ' Teacher', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
...
<br /><h2><?php echo Yii::t('app','{relation} that belongs to this {model}',array('{relation}'=>'App', '{model}'=>'Teacher'));?>: </h2>
<ul><?php foreach($model->apps as $foreignobj) { 

				printf('<li>%s</li>', CHtml::link($foreignobj->name, array('app/view', 'id' => $foreignobj->id)));

				} ?></ul><br /><h2><?php echo Yii::t('app','{relation} that belongs to this {model}',array('{relation}'=>'Group', '{model}'=>'Teacher'));?>: </h2>
